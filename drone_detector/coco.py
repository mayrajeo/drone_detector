...
class COCOProcessor():
    ...
    def coco_to_shp(self, coco_path:str=None, outdir:str='predicted_vectors'):
        """Generates shapefiles from json-file with coco annotations.
    ...
    def preds_to_coco(self, samples:list, preds:list, outfile:str='preds.json'):
        "Process list of IceVision `samples` and `preds` to COCO-annotation polygon format"
        outdict = self.coco_dict.copy()
            ...
            for j in rangeof(p['masks']):
                ...
                ann_dict = {
                    ...
                    'bbox': [p['bboxes'][j].xmin.item(),
                             ...
                             p['bboxes'][j].xmax.item() - pred['bboxes'][j].xmin.item(),
                             p['bboxes'][j].ymax.item() - pred['bboxes'][j].ymin.item()]
                }

                anns.append(ann_dict)
                outdict['annotations'].extedn(anns)
