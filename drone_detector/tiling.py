...
__all__ = ['make_grid', 'Tiler', 'untile_vector']
...
class Tiler():
    ...
    def tile_and_rasterize_vector(self, class_map:dict) -> None:
        "Rasterizes vectors based on tiled rasters. `class_map` should contain {'class':identifier} -info for labels"
        ...
        for r, v in tqdm(zip(raster_files, vector_files)):
            ...
            #for i in range(source_vector_layer.GetFeatureCount()):
            #    fet
            #for k, v in class_map.items():
            #    source_vector_layer.SetAttributeFilter(f"'label'='{k}'")
            gdal.RasterizeLayer(output_raster, [1], source_vector_layer, options=['ATTRIBUTE=label_id'])#burn_values=[v])
